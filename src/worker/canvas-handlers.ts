...
const ASYNC_METHODS: ReadonlySet<string> = new Set(['toBuffer', 'toURL', 'toFile', 'saveAs', 'toSharp'])
...
export function createCanvasHandlers(deps: CanvasHandlerDeps) {
  ...
  return {
    ...
    async callOnCanvas(canvasId: number, method: string, args: unknown[]): Promise<Buffer | string | void> {
      ...
      const canvas = mustGet(canvasId)
      const result = (canvas[method as keyof Canvas] as (...a: unknown[]) => unknown).apply(canvas, args)
      // toSharp hands back a Sharp, which cannot cross a thread boundary; it is drained to a
      // Buffer here to preserve this line's existing contract.
      return method === 'toSharp' ? (result as { toBuffer(): Promise<Buffer> }).toBuffer() : (result as Promise<Buffer | string | void>)
    },
