...
describe('createCanvasHandlers', () => {
  ...
  /**
   * A Sharp instance cannot cross a thread boundary, so it is drained to a Buffer inside the
   * worker. That preserves this line's existing contract; 5.0.0 returns the Sharp itself.
   */
  it('delegates toSharp and drains it to a buffer', async () => {
    const mockCanvas = {
