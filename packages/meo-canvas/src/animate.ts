/**
 * Easing curves, exact to the bit against `crates/meo-canvas-core/tests/assets/animate/*.tsv`,
 * recorded from v9.0.2's built module. The input is clamped and the output is not, and each
 * expression is grouped as v9 groups it: reassociating one changes the last bit the vectors pin.
 */
...
/**
 * Turns an ease-in curve into its out and in-out counterparts by reflection, as v9
 * writes them: `1 - easeIn(1 - t)` is not the closed form's float. `outQuart(0.1)`
 * written closed is `0.3439`, and the vectors hold `0.3438999999999999`.
 */
 ...
 * The iteration cap and epsilon are v9's, and the recorded vectors pin them: a
 * different cap converges to a different last bit.
 */
...
/** A spring's physical description. */
...
/**
 * The band around `zeta = 1` treated as critical damping. At exactly 1 both other
 * branches divide by zero, and `stiffness: 100, damping: 20` lands there, so round
 * numbers reach it.
 */
...
/**
 * Refuses a spring whose own `from`/`to` would be discarded: a track or sequence
 * step drives it over `0..1`, so they cannot be honoured, and dropping them would
 * animate to a value the caller never asked for.
 */
...
export interface TrackConfig<T extends Animatable> {
   ...
   * honoured; the type refuses one at compile time, and the assertion `track`
   * runs refuses it for JavaScript callers.
   */
...
/**
 * A colour, or a refusal saying what arrived. A string is almost certainly a v9
 * `mixColor` call, which took CSS strings and would otherwise yield `NaN` channels
 * that `formatColor` writes as a valid string painting nothing; a non-finite
 * channel is refused for the same reason.
 */
 ...
 *
 * **Takes {@link Rgba}, not strings.** One colour parser serves both surfaces and
 * lives in `meo-canvas-core`, so the string boundary is {@link parseColor} rather
 * than every function that touches a colour. A v9 call passing strings is refused
 * with a pointer to the migration guide, naming each argument that was a string.
 *
 * **`t` is not clamped**, because CSS interpolates colour through an overshooting
 * curve and clamps where the colour becomes paint — so an `outBack` driving a
 * colour may overshoot here, exactly as it does for a number. {@link formatColor}
 * is where an out-of-gamut result stops being silent.
 */
