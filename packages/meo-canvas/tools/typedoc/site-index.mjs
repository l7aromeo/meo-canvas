// Usage: node site-index.mjs <site-dir>. Writes the reference's landing page from
// the version directories present. `latest/` copies the newest version without a
// prerelease part, or is absent and the newest prerelease is linked by version. No
// dates: nothing offline records when a version was published.
...
// `latest` is a copy rather than a symlink, which a Pages tarball cannot carry, and
// the stamp beside it is the only record of which version it is: no stamp, no
// `latest/`.
const stampFile = join(site, 'latest', 'VERSION')
const latest = existsSync(stampFile) ? readFileSync(stampFile, 'utf8').trim() : null

// The rule, asserted: `docs.yml`'s assemble step decides `latest/` by copying and
// this by describing, so a stamped prerelease fails here.
