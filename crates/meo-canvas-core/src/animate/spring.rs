...
//!
//! Closed form rather than integrated step by step, so any page can be
//! evaluated on its own, in any order, with the same answer twice. A stepwise
//! simulation needs every earlier frame first.
//!
//! The three regimes are different solutions of one equation: underdamped
//! oscillates, critically damped is the fastest approach that does not, and
//! overdamped crawls in without crossing.
//!
...
#![expect(
    ...
    reason = "`!(stiffness > 0.0)` is deliberate and not `<=`: it refuses a \
              ...
              scan walks `t += 1/240` because that is what v9 walks -- see \
              `SETTLE_STEP_SECONDS`."
)]
#![expect(
    clippy::suboptimal_flops,
    reason = "a fused multiply-add rounds once where JavaScript rounds twice, \
              and this module is compared bit-for-bit against v9's own \
...
/// The spring v9 defaults to, and the one every caller gets unasked.
...
/// How close to `zeta == 1` counts as critically damped. A band, not an
/// equality: just under one the underdamped form divides by `omega_d`, which is
/// nearing zero, and the arithmetic fails before the physics does.
const CRITICAL_BAND: f64 = 1e-4;

/// A spring's physics, with no range of its own.
///
/// Split from [`Spring`] because a track supplies the range: a track takes a
/// `Shape`, which has nowhere to put `from` or `to`, so a range the track would
/// ignore cannot be written.
...
/// The scan's step, accumulated rather than multiplied: repeated addition of a
/// value with no exact binary form does not land where `n / 240` lands, and
/// v9's `t += 1/240` is the sequence both surfaces reproduce.
...
impl Spring {
    ...
    ///
    /// A spring approaches its target asymptotically, and this turns that into
    /// the number a page count needs. It walks the closed form rather than
    /// solving the envelope, which is only an upper bound: an underdamped
    /// spring can sit inside it while still swinging through the target.
    ///
    /// # Why a window and not a sample
    ///
    /// An underdamped spring passes through the target twice per cycle, so one
    /// sample near it proves nothing. A whole cycle inside the threshold does,
    /// because the envelope decays monotonically. Without oscillation there is
    /// no cycle to wait out, and two samples do.
    ///
    /// # Errors
    ///
    /// As [`Spring::at`], and [`Error::Spring`] for a `rest_delta` that is not
    /// greater than zero.
    pub fn settles_after(self, rest_delta: f64) -> Result<f64, Error> {
        // A threshold of zero asks for exact rest, which the closed form never
        // reaches, and a negative one asks nothing; both would scan to
        // `MAX_SETTLE_SECONDS` and report it as a settling time. Negated so
        // `NaN` is refused too, as the stiffness and mass checks are.
        if !(rest_delta > 0.0) {
...
mod tests {
    ...
    fn a_rest_delta_that_is_not_a_threshold_is_refused() {
        // Each of these would scan to `MAX_SETTLE_SECONDS` and answer a number
        // shaped like a settling time. Zero asks when the spring is exactly at
        // its target, which the closed form approaches without reaching.
