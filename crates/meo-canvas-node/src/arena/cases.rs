//! Emits `fixtures/arena-cases.json`: per property of the four
//! [`arena_group!`](super::group::arena_group) tables and per node kind (keyed
//! `__kind_*`), a probe scene -- Rust field names, wire-order values -- and the
//! bytes the codec writes. Regenerated by `just arena-cases`, never edited.
...
/// The prefix a node-kind case's key carries: two underscores, as [`ALL_KEY`]
/// has, since no Rust field begins with them and a kind case can never collide
/// with a property case.
...
impl ToJson for ImageSource {
    fn to_json(&self) -> String {
        match self {
            ...
            // `canonical()` rather than the list as authored: it is what the
            // bytes hold. The key is written even when empty, so a reader that
            // drops headers is red on every url case.
...
/// A segment's style as the properties it sets, walked from the [`text`] table
/// so a property added there appears on the next regeneration; unset fields
/// are omitted rather than written as fifteen nulls.
...
/// Standard base64, which is how bytes reach JSON: sixteen lines rather than a
/// dependency every consumer of the addon would carry for one encoder.
...
/// The node kinds a case is built for, each payload setting every field the
/// format writes: one per [`NodeTag`] (`every_node_kind_has_a_case`), one per
/// [`ImageSource`] variant, since a string and a byte buffer are different wire
/// shapes, and both arms of the text discriminant.
fn kind_cases() -> Vec<KindCase> {
    ...
    vec![
        ...
        // The markup arm of the text discriminant: a probe writing the string
        // must produce the runs Rust parses out of it. It parses into
        // something no other case encodes, since equal bytes would
        // mean one of two cases writes nothing.
        KindCase::markup(
        ...
        // Every step of the canonical form is load-bearing: drop the sort,
        // lower-casing or combining and this goes red, and the case-variant
        // repeat reddens lower-casing after combining. The TypeScript twin
        // stays an array of pairs, which can repeat a name.
        KindCase::built(
...
fn path_cases() -> Vec<KindCase> {
    let gradient = Gradient {
        // The endpoint arm, reached nowhere else in the artefact. Quarters
        // rather than whole percentages: `Percent(1.0)` is where a
        // hundredfold units error encodes identically, and a quarter is
        // exact in an `f32`.
    ...
    vec![
        ...
        // The gradient arm of the paint tag, kept apart because a gradient has
        // no spelling on the TypeScript surface: folded into the case
        // above, it would make the whole path payload unreachable from
        // that side.
        KindCase::built(
...
struct KindCase {
    ...
    kind: NodeKind,
    /// The markup a probe writes instead of building the payload: `Some` for
    /// the case that exercises the text discriminant, since writing the
    /// runs directly gives the same bytes without ever setting it.
...
impl KindCase {
    ...
    /// A case whose payload the probe writes as markup, its segments from
    /// [`meo_canvas_core::markup::parse_paragraph`] -- the function the decoder
    /// calls -- so the expectation is what the parser actually produces.
...
/// One node kind's payload as JSON, hand-written because [`json_tagged`]
/// carries one value per variant and these carry up to nine. Fields are named
/// in wire order, as the grammar in [`crate::arena`] lists them.
#[expect(
...
mod tests {
    ...
    /// Writes the artefact. Ignored by default, since it writes a tracked file:
    /// `just arena-cases` runs it, and `just arena-cases-check` runs it to a
    /// temp path and diffs rather than asking git, which reports untracked,
    /// written and staged alike.
    #[test]
    ...
    /// The text discriminant has a case for each arm: both reach the same
    /// `Scene`, so one alone would pass while half the format went
    /// unwritten.
    #[test]
    ...
    /// Every node kind has a case, so a kind added to [`NodeTag`] fails here
    /// rather than shipping a payload nothing encodes.
    #[test]
    ...
    /// Every `ImageSource` variant has a case: one node kind but three wire
    /// shapes, so a writer wrong about the buffer would pass on the path case.
    #[test]
    ...
    /// No two cases carry the same bytes: identical bytes would mean one of
    /// them wrote nothing, caught against the encoded output.
    #[test]
