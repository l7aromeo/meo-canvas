...
filename into `samples/` — `chart_samples.png`, `sample_grids.png` and
`sample_nested_grids.png` — and never glob or clean, so this directory is out of
their reach by construction rather than by convention. Replacing an image here
means exporting a new one from the bot and committing it.
